...
#### Escalation routing — who handles what

Not every issue belongs at the Strategist level. The discipline that prevents the Strategist from becoming a bottleneck on work the Executor could ship faster:

| Issue shape | Correct handler | Why |
| :---- | :---- | :---- |
| Bug in a single repo with no cross-cutting implications (posting error, cron misfire, copy fix, UI glitch) | **Executor directly** | The Executor has production-state awareness the Strategist lacks. Strategist diagnosis is slower and often wrong. |
| Cross-repo coordination (protocol change, multi-repo migration, shared infrastructure change) | **Strategist** | Single source of design across repos prevents drift. |
| Architecture decision (data model, integration pattern, new family of work) | **Strategist** | Decisions encoded once, referenced by every repo that adopts them. |
| New project onboarding, family-wide standard, tooling decision | **Strategist** | Defines the structure the Executor will work inside. |
| Tier 0 hotfix on a single repo | **Executor directly** | Bleed first, paper trail via the [Tier 0 backfill protocol](#tier-routing). |
| "Something broke on <site>" with no cross-repo implication | **Executor directly** | If the Strategist diagnoses single-repo bugs, the loop slows to chat-paced; the Executor lives in the codebase. |

The principle: **route work to the officer closest to the battlefield.** The Strategist's leverage is in cross-repo design and protocol; spending it on single-repo bugs wastes both the round trip and the Strategist's context window. When an issue arrives, the routing question comes before the diagnosis question: *whose level is this?*

When routing is ambiguous (a bug that *might* be cross-cutting), the Executor begins triage and escalates only if the diagnosis reveals cross-repo shape. Escalation is explicit and goes in the checkpoint, the same as tier escalation.
...
### Layered protocol — the supporting disciplines

The four protocols above are the spine. As an enterprise grows, four more disciplines become load-bearing — each one a deep-dive in its own right.

- **[Architecture Graphs](architecture-graphs.md).** A Strategist designs from outside the codebase. Generated structure maps — modules, routes, env vars, cron jobs, external services — are the highest-leverage Strategist input after the `AGENT.md` cascade. Read first on any Tier 3 change; regenerated by the Executor as part of the QA gate when many files change.
- **[Autonomous Loops](autonomous-loops.md).** Anything that runs without a human in the room — crons, webhooks, long-running agents — needs a documented kill switch, a heartbeat that's actually watched, a named owner, and a dry-run first cycle. Loops carry tier discipline by blast radius.
- **[Secrets and Rotation](secrets-and-rotation.md).** Two AIs and a human share a codebase; none of them should ever see a production secret in plaintext. Names appear in handoffs; values live only in the operator's password manager and the hosting console. Every secret has a rehearsed rotation playbook and a leak protocol.
- **[Dependency Management](dependency-management.md).** The lockfile is the source of truth. The Strategist designs against the lockfile, not the manifest. Upgrade cadence is declared per release tier; majors are Tier 3. A monthly audit catches divergence, vulnerabilities, and abandonment.
- **[Cost Management](cost-management.md).** Every recurring spend has a named owner and a budget. Spend reviews happen on a fixed cadence; overruns trigger the same backfill discipline as a Tier 0 hotfix. Circuit breakers fail closed.

These five layered disciplines connect into the spine the same way the others do: they ride on the four foundations, they accumulate evidence in changelogs, and they earn their place by surviving real failures. Adopt them when your shape demands them — not before.
...
- [Getting Started](getting-started.md) · [The Two Roles](two-roles.md) · [The Context Cascade](context-cascade.md) · [File Handling](file-handling.md) · [Checkpoints & Handoffs](checkpoints-handoffs.md) · [The Sanity Check](sanity-check.md) · [Bounded Deviation](bounded-deviation.md) · [Governance Sync](governance-sync.md) · [The Roadmap System](roadmap-system.md) · [The QA Gate](qa-gate.md) · [The Delta Log](delta-log.md) · [Architecture Graphs](architecture-graphs.md) · [Autonomous Loops](autonomous-loops.md) · [Secrets and Rotation](secrets-and-rotation.md) · [Dependency Management](dependency-management.md) · [Cost Management](cost-management.md)
